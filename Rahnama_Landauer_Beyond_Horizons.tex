...
72 falsifiable checks across 8 groups.
\end{abstract}
...
masses, so the numerical proximity should be treated with caution:
it could shift with physical quark masses or improved lattice data.
...
masses~\cite{Bazavov2016} confirm that the static-quark entropy peaks
sharply near the chiral crossover temperature
$T_c \approx 155$~MeV, consistent with the pattern seen here.
Extending the $\RLs$ analysis to these data is a natural
next step.
\end{enumerate}
...
72 falsifiable checks across 8 groups, including negative tests that
