...
Section~\ref{sec:string_tension} illustrates the framework with a
dimensional-analysis estimate of the QCD string tension.
...
% ============================================================
\section{Dimensional-analysis illustration: QCD string tension}
\label{sec:string_tension}

A one-dimensional source (flux tube) produces a constant force,
i.e., a linear potential $\Phi(r) = \sigma r$ with string tension
$\sigma$.  Identifying the source strength with the Landauer
maintenance cost per bit relates $\sigma$ to the information density
along the tube:
\begin{equation}
...
of $\sigma$ from the Landauer framework.  It is a dimensional-analysis
illustration: given the measured string tension and critical
temperature, the implied information density is a positive number of
order unity in natural units (a few bits per fermi, exceeding the bare
color content).  Any ratio of two positive physical quantities with
the same dimensions yields a positive number; the value becomes
informative only insofar as it is comparable to independently
motivated scales (here, the color content $\log_2 3 \approx 1.58$
bits).
...
a dimensional-analysis illustration consistent with QCD phenomenology.
\end{enumerate}
